...
Program Algorithm: The program has the following underlying logic -
				...
				1. At first, all the input text files are parsed line by line into a list.
				2. On each line in the above list, tokenization using regular expressions are performed so as to generate a final list of tokens.
	...
	for i in range(int(sys.argv[2])):
		...
		while start_word not in boundary_terms: 
			...
			if sent_words == []:
				...
				for i in prob_for_each_bigram:
					diff_list.append(abs(r-i))
	...
	for i in range(int(sys.argv[2])):
		...
		while start_word not in boundary_terms: 
			...
			if sent_words == []:
				...
				for i in prob_for_each_trigram:
					diff_list.append(abs(r-i))
